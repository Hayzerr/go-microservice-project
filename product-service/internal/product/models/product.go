...
type Product struct {
	ID          string      `json:"id"`           // Уникальный идентификатор продукта (например, UUID)
	...
	FestivalID  string      `json:"festival_id"`  // ID фестиваля, к которому относится продукт (если применимо)
